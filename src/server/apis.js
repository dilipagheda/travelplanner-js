...
async function getData(placeName, date) {
  ...
  if (!momentDate.isValid()) {
    throw new BadRequest(
      400,
      "Invalid date. Please send date in format YYYY-MM-DD"
    );
  }
  ...
  if (isTravelDateWithin7Days) {
    weatherBitData.current = await getWeatherBitDataCurrent(
      placeName,
      geoNamesData.countryCode
    );
  } else {
    weatherBitData.forecast = await getWeatherBitDataForecast(
      placeName,
      geoNamesData.countryCode
    );
  }
...
async function getGeoNamesData(placeName) {
  ...
  const geoData = await fetch(
    `${GEONAMES_BASEURL}?q=${placeName}&maxRows=1&username=${GEONAMES_USERNAME}`
  )
    .then((response) => {
      if (!response.ok) {
...
async function getWeatherBitDataCurrent(placeName, countryCode) {
  ...
  const weatherBitData = fetch(url)
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          "sorry! something went wrong from weatherbit current API"
        );
      }
      return response.json();
    })
...
async function getWeatherBitDataForecast(placeName, countryCode) {
  ...
  const weatherBitData = fetch(url)
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          "sorry! something went wrong from weatherbit forecast API"
        );
      }
      return response.json();
    })
