...
const clickHandler = async (event) => {
    ...
    if(!placeName || !date) {
        document.querySelector('.my-trips').innerHTML = showError('Please enter valid values!')
        return
    }

    const momentDate = moment(date, "DD-MM-YYYY");

    const fullUrl = `${baseUrl}/travel?placeName=${placeName}&date=${momentDate.format('YYYY-MM-DD')}`
